...
exports.ClassFileParser = undefined;

var _binaryParser = require('binary-parser');

var _ConstantPoolParser = require('./ConstantPoolParser');

var _ClassMembers = require('./ClassMembers');
 ...
 * @type {Object}
 */
var ClassFileParser = exports.ClassFileParser = _binaryParser.Parser.start()
// This is the default endian type for binary-parser but it's better to be safe than sorry.
.endianess('big').uint32('magic', { assert: JVM_CLASS_FILE_MAGIC_NUMBER }).uint16('minor_version').uint16('major_version').uint16('constant_pool_count').array('constant_pool', {
  type: _ConstantPoolParser.ConstantPoolInfo,
  length: function length() {
    var lastIdx = this.constant_pool.length - 1;
    var lastEntry = this.constant_pool[lastIdx];
...
}).uint16('access_flags').uint16('this_class').uint16('super_class').uint16('interface_count').array('interfaces', {
  type: _ClassMembers.InterfaceInfo,
  length: function length() {
    return this.interface_count;
  }
}).uint16('field_count').array('fields', {
  type: _ClassMembers.ClassMemberInfo,
  length: function length() {
    return this.field_count;
  }
}).uint16('method_count').array('methods', {
  type: _ClassMembers.ClassMemberInfo,
  length: function length() {
    return this.method_count;
  }
}).uint16('attribute_count').array('attributes', {
  type: _ClassMembers.AttributeInfo,
  length: function length() {
